// games/uraomote: リズム天国「ウラオモテ」風ミニゲーム。表拍(kind=0)と裏拍(kind=1)が
// セクションごとに入れ替わる譜面（BPM116）を、metronome と同じ engine/scene-kit 疎結合契約で実装する。
...
import midiUrl from "../../../charts/uraomote.mid?url";
import sf2Url from "../../../charts/dev.sf2?url";
import overlay from "../../../charts/uraomote.json";

import { UraomoteScene } from "./scene";
...
/** approachSec = 1.55（≒3 拍 @116BPM = 60/116×3）。scene.ts の APPROACH_SEC と一致させること。 */
...
/** `Minigame` 契約の uraomote 実装。 */
class UraomoteGame implements Minigame {
  readonly id = "uraomote";
  ...
  createScene(canvas: HTMLCanvasElement): MinigameScene {
    const scene = new UraomoteScene(canvas);
    this.#scene = scene;
    this.#canvas = canvas;
    return scene;
  }
  ...
  start(): void {
    ...
    if (ctx === null || songBuffer === null || tapBuffer === null || canvas === null) {
      throw new Error("uraomoteGame.start(): load() と createScene() を先に呼ぶこと");
    }
...
const uraomoteGame: Minigame = new UraomoteGame();
export default uraomoteGame;
