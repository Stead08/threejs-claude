...
- 長さ: 64 小節 = 256 拍（= 128 秒）。
- Track 0: テンポ・拍子メタのみ。
- Track "CLICK"（トラック名メタ）: ch9（パーカッション）。拍 0 から毎拍 key 37 vel 100、小節頭のみ key 38 vel 127（37 の代わり）。ノート長 1/8 拍。
- Track "CUES"（トラック名メタ）: ch0。**カウントイン 2 小節（8 拍）の後**、拍 8 から毎拍 key 36、小節頭は key 38。ノート長 1/8 拍。**このトラックはレンダ時に除外される**（§3.2）。
- カウントイン: 冒頭 2 小節は CLICK のみでキューを置かない。曲頭キューにも全接近時間（approachSec）とテンポの手がかり（8 クリック）を保証するため（レビュー指摘による追加。これが無いと曲頭キューが構造的に判定不能でパーフェクト勲章が取得できない）。
...
- bank 0, preset 0 "Lead": 単サイクル矩形波サンプルのループ再生（M0 では未使用でも定義する）
- bank 128, preset 0 "Percussion": key 36 = 低いクリック、key 37 = クリック、key 38 = アクセント（高め/強め）。短い減衰音（sine バースト or ノイズバースト、~50-150ms）
