...
- Track 0: テンポ・拍子メタのみ。
- Track "CLICK"（トラック名メタ）: ch9（パーカッション）。毎拍 key 37 vel 100、小節頭のみ key 38 vel 127（37 の代わり）。ノート長 1/8 拍。
- Track "CUES"（トラック名メタ）: ch0。毎拍 key 36、小節頭は key 38。ノート長 1/8 拍。**このトラックはレンダ時に除外される**（§3.2）。
