...
- プレイ環境: **スマホ縦持ち（9:16〜9:19.5）**、片手親指プレイを基本形に据える（仮決定 → §9）
- 3D の使い所: 奥行き方向から物が飛んでくる構図は縦画面と相性が良い。カメラワーク・ローポリトゥーンの「1 ボタンでも画面が豪華」を狙う
- v1 スコープ: **ミニゲーム 1 本**（題材は「飛んでくる物を打ち返す」カラテ家型を仮置き）+ タイトル/設定/リザルト
...
**路線 B（v1 後半〜v2 の選択肢）**: シグネチャ音（キー音・効果音）を fundsp 等で**自作シンセ合成**。ローポリ・トゥーンの絵にはチップチューン寄りの音も合う。SoundFont の品質・サイズ問題を根本回避できる。まず路線 A（SF2）で立ち上げ、音の方向性を見て判断。
...
- ベース素材はライセンスの寛容な GeneralUser GS 等から選定（M0 で試聴・確認）
...
`games/karate`（仮）: キュー音・3D 演出・ヒット VFX/SFX・リザルト・ランクまで通し、**「気持ちよさ」を実機で徹底チューニング**。
...
## 9. 残る仮決定（異議がなければこのまま進む）

1. **縦持ち固定** — 片手プレイ優先で縦を基本形に。横持ち対応はやらない前提で 3D 構図を作る
2. **v1 の題材** — 「飛んでくる物を打ち返す」カラテ家型（3D 演出が映える・動詞がタップ 1 個）
3. **音の方向性** — まず SF2（GM 風）で立ち上げ、チップチューン寄り自作音色（路線 B）は音を聞いてから判断
